...
3. **Backend neutrality.**
   ...
   NumPy or PyTorch without replanning. Other Array API namespaces use the
   same path only when their namespace satisfies the capabilities required
   by every selected runtime step.
